...
var urlPrefix = G.CloudDomain;
var IMAGEKEY = G.ImageKey;
...
var o = {
	...
	images: { "default": [] }, // 所有图片 images {"albumId": []}
	albums: [], // 相册
	isAddAlbum: false,
	...
	pageAddAlbum: function (albumId) {
		var html = '<option value="' + albumId + '">' + albumId + '</option>';
		$("#albumsForUpload").append(html).val(albumId);
		$("#albumsForList").append(html);
	},
	pageUpdateAlbum: function (albumId, albumName) {
		$('option[value="' + albumId + '"]').html(albumName).val(albumName);
	},
	toggleAddAlbum: function () {
	...
	processAlbum: function () {
		...
		$("#addAlbumBtn").click(function () {
			self.toggleAddAlbum();
			$("#addOrUpdateAlbumBtn").html(getMsg("Add Album")).prop('disabled', true).hide();
			$("#albumName").val("").focus();
			self.isAddAlbum = true;
		});
		...
		$("#addOrUpdateAlbumBtn").click(function () {
			...
			if (self.isAddAlbum) {
				// $.get("/cloud/addAlbum", {name: albumName}, function(ret) {
				// 	if(ret.Ok) {
				// 		$("#albumName").val("");
				...
				$.get("/cloud/renameAlbum", { curName: curAlbum, newName: albumName }, function (ret) {
					if (ret.Ok) {
						$("#albumName").val("");
						self.showMsg(getMsg("Rename Success!"));
						self.pageUpdateAlbum(curAlbum, albumName);

						self.images[albumName] = self.images[curAlbum];
						delete self.images[curAlbum];
						for (var i in self.images[albumName]) {
							self.images[albumName][i][IMAGEKEY] = self.images[albumName][i][IMAGEKEY].replace(curAlbum + "/", albumName + "/");
						}
						for (var i in self.albums) {
							if (self.albums[i] == curAlbum) {
								self.albums[i] = albumName;
							}
						}
						// log(self.images);
						// log(self.albums);
						var albumId = $("#albumsForList").val();
						if ((albumId == albumName) || (albumId.length == 0)) {
		...
		$("#deleteAlbumBtn").click(function () {
			...
			$.get("/cloud/deleteAlbum", { albumId: albumId }, function (ret) {
				if (typeof ret == "object" && ret.Ok == true) {
					...
					// self.images.removeIf(function (e, i) {
					// 	return e[IMAGEKEY].startsWith(albumId + "/");
					// });
					self.images[albumId] = [];
	...
	renderAlbums: function () {
		...
		for (var i in self.albums) {
			var option = '<option value="' + self.albums[i] + '">' + self.albums[i] + '</option>';
			html += option;
		}

		$("#albumsForUpload").html(html); // append
		$("#albumsForList").html(html);
	},
	...
	getImages: function () {
		let self = this;
		self.loadingStart();
		$.get("/cloud/getImages", {}, function (ret) {
			if (!ret || !ret.Count) {
				self.noImages();
				return;
			}
			...
			if (G.getImagesCallback) {
				ret.List = G.getImagesCallback(ret.List);
			}

			let sortKey = G.SortKey;
			let sortRule = G.SortRule;
			ret.List.sort(function (a, b) {
				if (a[sortKey] < b[sortKey]) return -sortRule;
				if (a[sortKey] > b[sortKey]) return sortRule;
				return 0;
			});
			...
			for (var i in ret.List) {
				let keyList = ret.List[i][IMAGEKEY].split("/");
				if (keyList.length > 1) {
					if (self.albums.indexOf(keyList[0]) < 0) {
						self.albums.push(keyList[0]);
					}
					if (typeof self.images[keyList[0]] === "undefined") {
						self.images[keyList[0]] = [];
					}
					self.images[keyList[0]].push(ret.List[i]);
				} else {
					self.images["default"].push(ret.List[i]);
				}
			}
			// log(self.images);
			// log(self.albums);
			...
			self.renderAlbums();
			self.renderImages($("#albumsForList").val(), 1, true, $("#key").val());
		});
	},

	modifyTokenTime: function() {
		var self = this;
		if (G.ImageBedType != localStorage.getItem("ImageBedType")) {
			for (var i in self.albums) {
				localStorage.setItem(self.albums[i]+"tokenTime", "1");
			}
			localStorage.setItem("ImageBedType", G.ImageBedType);
		}
	},
	...
	renderImages: function (albumsId, page, needRenderPagination, key) {
		var self = this;
		if (!page) {
			page = 1;
		}
		if (!albumsId) { albumsId = "default"; }
		if (self.images[albumsId].length < 1) { return }
		...
		let html = "";
		let count = 0;
		for (var i in self.images[albumsId]) {
			var each = self.images[albumsId][i];
			// if (albumsId && !each[IMAGEKEY].startsWith(albumsId + "/")) { continue }

			let keyList = each[IMAGEKEY].split("/");
			let titleList = keyList[keyList.length - 1].split(".");
			let title = titleList.length == 1 ? titleList[0] : titleList.slice(0, titleList.length - 1).join("."); // slice [start, end)
			if (title.length == 0) { continue }
			if (key && !title.includes(key.toLowerCase())) { continue } // search key

			// 分页
			count++;
			if (count <= (page - 1) * G.perPageItems) { continue }
			if (count > page * G.perPageItems) { continue }

			var classes = "";
			var src = urlPrefix + "/" + each[IMAGEKEY]
			// log(src);
			if (selectedMap[each[IMAGEKEY]]) {
			...
			html += '<div class="tools clearfix" data-id="' + each[IMAGEKEY] + '"><div class="file-title pull-left">' + title + '</div><div class="pull-right"><a href="javascript:;" class="del" data-id="' + each[IMAGEKEY] + '"><span class="fa fa-trash"></span></a></div></div>';
			html += "</li>";
		}
	...
	removeSelectedImage: function (src) {
		var self = this;
	...
	init: function () {
		...
		$("#albumsForList").change(function () {
			...
			self.renderImages(albumId, 1, true, $("#key").val());
		});
		...
		$("#imageList").on("click", '.del', function (e) {
			...
			if (confirm(getMsg("Are you sure to delete this image ?"))) {
				...
				$.get("/cloud/deleteImage", { key: fileId }, function (ret) {
					if (ret.Ok) {
						var $li = $(t).closest('li');
						if ($li.hasClass("selected")) {
							self.removeSelectedImage(G.CloudDomain+"/"+fileId);
						}
						$(t).closest('li').remove();

						let albumId = getAlbumIdByKey(fileId);
						self.images[albumId] = self.images[albumId].filter(item => item[IMAGEKEY] != fileId);
						self.renderImages($("#albumsForList").val(), self.pageNum, true, $("#key").val());
					}
				});
			}
		});
		...
		$("#imageList").on("click", '.file-title', function (e) {
			...
			$input.blur(function () {
				let t = this;
				let title = $(t).val();
				let fileId = $(p).parent().data('id');
				let albumId = getAlbumIdByKey(fileId);
				let pathList = fileId.split("/");
				if (pathList.length > 1) {
					title = pathList[0] + "/" + title;
				}
				let fileNameList = fileId.split(".");
				let suffix = fileNameList[fileNameList.length - 1];
				title += "." + suffix;
				if (fileId == title) {
					return
				}
				$.post("/cloud/updateImageTitle", { old: fileId, new: title }, function (ret) {
					if (ret.Ok) {
						// $(p).parent().attr("data-id", title);
						// $(p).parent().data("id", title);
						// $(p).next().children().attr("data-id", title);
						// $(p).next().children().data("id", title);
						// $(p).html($(t).val());
						// let i = $(p).parent().prev().children()
						// $(i).attr("src", G.CloudDomain + "/" + title);
						// $(i).attr("data-original", G.CloudDomain + "/" + title);
						for (var idx in self.images[albumId]) {
							if (self.images[albumId][idx][IMAGEKEY] == fileId) {
								self.images[albumId][idx][IMAGEKEY] = title;
							}
						}
						self.renderImages($("#albumsForList").val(), self.pageNum, true, $("#key").val());
					}
		...
		self.getImages();
		self.modifyTokenTime(); // 如果 imageBedType不匹配，需要重新获取upToken
		self.initUploader();
	},
	...
	uploadRefreshImageList: function (albumName, image) {
		var self = this;
		// check albumId
		if ($("#albumsForList").val() == albumName) {
			self.needRefresh = true;
		}
		image["putTime"] = new Date().getTime() * 10000;

		if (!albumName) {albumName = "default";}
		image = [image];
		if (G.getImagesCallback) {
			image = G.getImagesCallback(image);
		}
		self.images[albumName].splice(0, 0, image[0]); // 在第0个位置插入
	},

	uploadImage: function (file, data) {
		let self = this;

		albumName = self.isAddAlbum ? $("#albumName").val() : $("#albumsForUpload").val();

		function _success(e, d, ret, albumName) {
			if (e.isAddAlbum) {
				e.isAddAlbum = false;
				$("#addOrUpdateAlbumBtn").prop('disabled', false).show();
				e.toggleAddAlbum();
				if (albumName) {
					$("#albumName").val("");
					e.pageAddAlbum(albumName);
					e.images[albumName] = [];
				}
			}

			d.context.remove();
			e.addSelectedImage(G.CloudDomain + "/" + ret[IMAGEKEY]); // add image to preview
			e.uploadRefreshImageList(albumName, ret); // reresh image list
		}

		function _error(d) {
			d.context.empty();
			var tpl = $('<li><div class="alert alert-danger"><a class="close" data-dismiss="alert">×</a></div></li>');
			tpl.find('div').append('<b>Error:</b> ' + d.files[0].name + ' <small>[<i>' + formatFileSize(d.files[0].size) + '</i>]</small> ' + d.errorThrown);
			d.context.append(tpl);
			$("#upload-msg").scrollTop(1000);
		}

		if ((new Date().getTime() - localStorage.getItem(albumName + "tokenTime")) < G.TokenExpire) { // Token 没有过期
			// _uploadImage(localStorage.getItem(albumName + "upToken"));
			G.uploadImage(localStorage.getItem(albumName + "upToken"), file, albumName, self, data, _success, _error);
		} else {
			$.get("/cloud/genUpToken", { albumName: albumName }, function (ret) {
				if (ret.length > 0) {
					localStorage.setItem(albumName + "tokenTime", new Date().getTime()) // 时间单位ms
					localStorage.setItem(albumName + "upToken", ret);
				}
				// _uploadImage(ret);
				G.uploadImage(ret, file, albumName, self, data, _success, _error);
			});
		}
	},
		...
		$('#upload').fileupload({
			...
			*/
			...
			add: function (e, data) {
				...
				self.uploadImage(data.files[0], data);
			},
